...
def long_utilization(user_id,app_type_id, ymd):
    '''
    ---last 4 weeks of estimated data hour wise and Inserting into BigTable of numpy array.
    Result : Data Type of numpy array values 'Integer'
    Return:
       BigTable insertion hour wise responses of numpy array.
    '''
    import msgpack 
    import settings
    reload(settings)
    ...
    day = 86400    #no.of seconds per day
    week = day*7
    ymd = int(ymd)
    ...
    today_ymd = ymd- (day*0)
    last_4weeks_ymd= today_ymd- (week *4)
    delta = today_ymd - last_4weeks_ymd
    delta_days = delta/day
    for i in range(1,delta_days+1):			#generate h_tag
        idate= last_4weeks_ymd + (day*i)
	...
	yyyymm= "{:%Y%m}".format(datetime.utcfromtimestamp(idate))
        ...
        if(cal_count != None and cal_count > 0):
    ...
    if(num_on_days !=0):
        ...
        for i in range(24):
            if(i <10):
                column = "l0{}".format(i)
                count=insertCell(table_id,user_id, app_type_id,ymd, 'Weekly', column, str(ut_pct[i]))
...
def main(spark,df_users):
    LOGGER.info('LongUtilization Execution started...')
    udf_long_utilization =udf(long_utilization,StringType())
    df_longUtilization = df_users.select("user_id", "app_type_id",udf_long_utilization(df_users.user_id,df_users.app_type_id, df_users.ymd).alias("longutilization"))
    df_longUtilization.show(100,False)
...
if __name__ == "__main__":
    ...
    spark = SparkSession \
        .builder.master("yarn") \
        .appName("LongUtilization") \
        .config("spark.sql.warehouse.dir", warehouse_location) \
        .getOrCreate()
    log4jLogger = spark.sparkContext._jvm.org.apache.log4j
    LOGGER = log4jLogger.LogManager.getLogger(__name__)
    LOGGER.info('LongUtilization script logger initialized at {}'.format(time.time()))
    ...
    LOGGER.info("=====LongUtilization Completed succesfully.=======")
    time4 = time.time()
